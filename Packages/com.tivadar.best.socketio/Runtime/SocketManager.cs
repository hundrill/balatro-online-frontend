...
namespace Best.SocketIO
{
    ...
    public sealed class SocketManager : IHeartbeat, IManager
    {
        ...
        /// </summary>
        public Action<SocketManager, IncomingPacket> OnIncomingPacket;
        ...
        void IManager.SendPacket(OutgoingPacket packet)
        {
            ...
            {
                if (packet.IsVolatile || this.State == States.Closed)
                {
                    BufferPool.Release(packet.PayloadData);
                    return;
                }
        ...
        void IManager.OnPacket(IncomingPacket packet)
        {
            ...
            try
            {
                this.OnIncomingPacket?.Invoke(this, packet);
            }
            catch(Exception ex)
            {
                HTTPManager.Logger.Exception(nameof(SocketManager), $"{nameof(this.OnIncomingPacket)}", ex, this.Context);
            }
        ...
        void IHeartbeat.OnHeartbeatUpdate(DateTime now, TimeSpan dif)
        {
            switch (State)
            {
                ...
                case States.Opening:
                    if (now - ConnectionStarted >= Options.Timeout)
                    {
                ...
                case States.Reconnecting:
                    if (ReconnectAt != DateTime.MinValue && now >= ReconnectAt)
                    {
                ...
                case States.Open:
                    ...
                    if (now - lastPingReceived > TimeSpan.FromMilliseconds(Handshake.PingInterval + Handshake.PingTimeout))
                        (this as IManager).TryToReconnect();
