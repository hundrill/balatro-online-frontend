...
namespace Best.SocketIO
{
    ...
    public struct EmitBuilder
    {
        ...
        internal Action<Socket> timeOutCallback;
        ...
        public EmitBuilder Timeout(TimeSpan timeout)
        {
            this.timeOut = timeout;
            return this;
        }

        public EmitBuilder Timeout(TimeSpan timeout, Action<Socket> callback)
        {
            this.timeOut = timeout;
        ...
        public Socket Emit(string eventName, params object[] args)
        {
            ...
            if (this.timeOut > TimeSpan.Zero && this.id != -1)
                Timer.Add(new TimerData(this.timeOut, this, OnTimeout));
        ...
        private static bool OnTimeout(DateTime now, object context)
        {
            var builder = (EmitBuilder)context;

            string name = IncomingPacket.GenerateAcknowledgementNameFromId(builder.id);
            if (builder.socket.TypedEventTable.Unregister(name) && builder.timeOutCallback != null)
            {
                try
                {
                    builder.timeOutCallback.Invoke(builder.socket);
                }
                catch (Exception e)
                {
                    HTTPManager.Logger.Exception(nameof(EmitBuilder), nameof(OnTimeout), e, builder.socket.Context);
                }
            }
    ...
    public sealed class Socket : ISocket
    {
        ...
        void ISocket.Open()
        {
            ...
            if (Manager.State == States.Open)
                OnTransportOpen();
            else if (Manager.Options.AutoConnect && Manager.State == States.Initial)
                Manager.Open();
        }
        ...
        public EmitBuilder Timeout(TimeSpan timeout)
        {
            return new EmitBuilder(this).Timeout(timeout);
        }

        public EmitBuilder Timeout(TimeSpan timeout, Action<Socket> timeoutCallback)
        {
            return new EmitBuilder(this).Timeout(timeout, timeoutCallback);
        }
        ...
        public void Off(string eventName)
        {
            ...
            if (eventName.Equals(EventNames.GetNameFor(SocketIOEventTypes.Connect), StringComparison.OrdinalIgnoreCase))
                this.On<ConnectResponse>(EventNames.GetNameFor(SocketIOEventTypes.Connect), OnConnected);
        }
        ...
        public void Off(SocketIOEventTypes type)
        {
            ...
            if (type == SocketIOEventTypes.Connect)
                this.On<ConnectResponse>(EventNames.GetNameFor(SocketIOEventTypes.Connect), OnConnected);
        }
